...
# Reasoning models wrap chain of thought in a model-specific tag (<think>, <reasoning>, ...)
# inside the normal content field. strip_reasoning() removes it; the tag set comes from
# REASONING_TAGS. Tool calls are parsed from the *stripped* content, so a tool call the model
# only considered inside its reasoning is never executed.
